 ...
 * the file. The fuzzy header matching below is lifted from an earlier
 * site-planning prototype (`normalise, strip punctuation, substring match in
 * priority order`) and generalised so any caller can supply its own column
 * spec instead of a hard-coded field list.
 *
