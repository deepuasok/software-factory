...
const SAMPLE_COMMENTS: ProvenanceComment[] = [
  { id: "c1", author: "Moyo Shobowale", body: "PSM moved up after the March site swap. Flagging for the study team to confirm.", at: new Date(Date.now() - 2 * 3600000).toISOString() },
  { id: "c2", author: "Derek Wang", body: "Confirmed with CSM — leave as is.", at: new Date(Date.now() - 26 * 3600000).toISOString(), resolved: true },
];

const SAMPLE_ACTIVITY: ActivityEntry[] = [
  recordChange({ verb: "changed", actor: "Moyo Shobowale", field: "PSM", from: "0.42", to: "0.50", at: new Date(Date.now() - 2 * 3600000).toISOString() }),
  recordChange({ verb: "changed", actor: "CATALYST model", field: "Tier", from: "Tier 2", to: "Tier 1", at: new Date(Date.now() - 20 * 3600000).toISOString() }),
  recordChange({ verb: "changed", actor: "Derek Wang", field: "Owner", from: "Unassigned", to: "Moyo Shobowale", at: new Date(Date.now() - 30 * 3600000).toISOString() }),
];
...
export function DetailPage({
  title = "Northgate General — Site M26-092",
        ...
        <div className="flex flex-col gap-4">
          <StatRow>
            <StatTile
              ...
              value="0.50"
              tone="ok"
              note={<SourceBadge sourceSystem="CATALYST model" modelVersion="2.4.1" confidence={0.86} />}
            />
            <StatTile
              label="Sites activated"
              value="84"
              note={<SourceBadge sourceSystem="TESLA" />}
            />
            <StatTile
              label="Patients enrolled"
              value="612"
              note={<SourceBadge sourceSystem="Go/Feas" />}
            />
            <StatTile
              label="Enrolment forecast"
              value="720"
              tone="warn"
              note={<SourceBadge sourceSystem="CATALYST model" modelVersion="2.4.1" confidence={0.71} />}
            />
          </StatRow>
