...
type ReconcileRow = {
  field: string;
  trusted: "forecastModel" | "supplyPlan" | "vendorModeler";
  forecastModel: SourceValue;
  supplyPlan: SourceValue;
  vendorModeler: SourceValue;
};

const SAMPLE_ROWS: ReconcileRow[] = [
  {
    field: "Enrolment forecast (patients)",
    trusted: "forecastModel",
    forecastModel: { sourceSystem: "Forecast model", value: "720", modelVersion: "2.4.1", confidence: 0.82 },
    supplyPlan: { sourceSystem: "Supply plan", value: "640" },
    vendorModeler: { sourceSystem: "Vendor modeler", value: "705", modelVersion: "1.9", confidence: 0.6 },
  },
  {
    field: "Sites needed",
    trusted: "forecastModel",
    forecastModel: { sourceSystem: "Forecast model", value: "84", modelVersion: "2.4.1", confidence: 0.9 },
    supplyPlan: { sourceSystem: "Supply plan", value: "90" },
    vendorModeler: { sourceSystem: "Vendor modeler", value: "88", modelVersion: "1.9", confidence: 0.55 },
  },
  {
    field: "Time to last patient in",
    trusted: "supplyPlan",
    forecastModel: { sourceSystem: "Forecast model", value: "38 weeks", modelVersion: "2.4.1", confidence: 0.64 },
    supplyPlan: { sourceSystem: "Supply plan", value: "41 weeks" },
    vendorModeler: { sourceSystem: "Vendor modeler", value: "36 weeks", modelVersion: "1.9", confidence: 0.48 },
  },
];
 ...
 *
 * Use it wherever the forecast model, the supply plan and the vendor modeler disagree on a
...
export function ReconcilePage({
  title = "Reconcile enrolment figures — ST-1042",
  ...
  const [activity, setActivity] = useState<ActivityEntry[]>([
    recordChange({ verb: "changed", actor: "Jordan Ellis", field: "Sites needed", from: "90 (Supply plan)", to: "84 (Forecast model)", at: new Date(Date.now() - 3600000).toISOString() }),
  ]);
  ...
  const columns: Column<ReconcileRow>[] = [
    ...
    { key: "forecastModel", header: "Forecast model", render: (r) => renderSourceCell(r.forecastModel, r.field, "forecastModel") },
    { key: "supplyPlan", header: "Supply plan", render: (r) => renderSourceCell(r.supplyPlan, r.field, "supplyPlan") },
    { key: "vendorModeler", header: "Vendor modeler", render: (r) => renderSourceCell(r.vendorModeler, r.field, "vendorModeler") },
  ];
