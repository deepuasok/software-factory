...
cp apps/<app-slug>/.env.example apps/<app-slug>/.env   # local only, gitignored
...
10. **A real `.env` never leaves the machine.** It stays gitignored; the repo
    ships `.env.example` with blank or harmless values. Any zip or handoff of an
    app excludes `.env`.
