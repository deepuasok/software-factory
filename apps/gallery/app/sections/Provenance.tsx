...
const USERS: ProvenanceUser[] = [
  { id: "jordan", name: "Jordan Ellis", hint: "Senior Associate, Site Planning" },
  { id: "marcus", name: "Marcus Hale", hint: "Director, Insights" },
  { id: "lena", name: "Lena Fischer", hint: "Staff Data Scientist" },
];

const SAMPLE_COMMENTS: ProvenanceComment[] = [
  { id: "c1", author: "Jordan Ellis", body: "Network activation pushed this up two points this week.", at: new Date(Date.now() - 5 * 3600000).toISOString() },
];
...
export function ProvenanceSection() {
  const [assignee, setAssignee] = useState<string | null>("jordan");
  const [comments, setComments] = useState<ProvenanceComment[]>(SAMPLE_COMMENTS);
  const [activity, setActivity] = useState<ActivityEntry[]>([
    recordChange({ verb: "changed", actor: "Jordan Ellis", field: "PSM", from: "0.42", to: "0.50", at: new Date(Date.now() - 3 * 3600000).toISOString() }),
  ]);
    ...
    <div className="flex flex-col gap-5">
            ...
            <StatTile
              ...
              note={
                <div className="flex items-center gap-1.5 flex-wrap">
                  <SourceBadge sourceSystem="Forecast model" modelVersion="2.4.1" confidence={0.86} />
      ...
      <Grid cols={2}>
        <Part name="Avatar" when="Use it anywhere a name needs a face and there is no photo.">
          ...
          <Avatar name="Lena Fischer" size="md" tone="ok" />
        </Part>
        ...
        <Part name="SourceBadge" when="Use it next to every number that did not come straight from a form.">
          <SourceBadge sourceSystem="Forecast model" modelVersion="2.4.1" confidence={0.86} />
          <SourceBadge sourceSystem="Supply plan" />
          <SourceBadge sourceSystem="Site tracker" />
        </Part>
      </Grid>
